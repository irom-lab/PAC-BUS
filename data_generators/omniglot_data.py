...
class OmniglotNShot:
    def __init__(self, root, batchsz, n_way, k_shot, k_query, imgsz=28,  nme=False, device=None):
        imgsz = imgsz
        k_query = k_query
        self.resize = imgsz
        self.num_train = 1200
        ...
        self.x_train, self.x_test = self.x[:self.num_train], self.x[self.num_train:]
        if device is not None:
            self.x_train = torch.from_numpy(self.x_train).to(device)
            self.x_train = torch.from_numpy(self.x_test).to(device)
        ...
        self.cls_members_train = []
        self.num_groups = 1
        self.device = device

        if nme:
            tr = [i for i in range(self.num_train)]
            np.random.shuffle(tr)
            for i in range(self.num_groups*n_way):
                part_size = len(tr) // (self.num_groups * n_way)
                self.cls_members_train.append(np.sort(tr[i*part_size:(i+1)*part_size]))
    ...
    def load_data_cache(self, data_pack):
        ...
        for i in range(self.batchsz):  # one batch means one set
            ...
            if self.nme and data_pack.shape[0] == self.num_train:
                lbls = np.array(self.cls_members_train)
                group = np.random.choice(self.num_groups)
                inds = [self.num_groups*i + group for i in range(self.n_way)]
                lbls = lbls[inds]
                selected_cls = np.array([np.random.choice(lbls[c]) for c in range(self.n_way)])
            else:
    ...
    def next(self, mode='train'):
        ...
        :param mode: The name of the splitting (one of "train", "test")
        :return:
        """
